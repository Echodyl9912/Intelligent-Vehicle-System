...
void MainWindow::layoutInit()
{
    ...
    pushButton[2]->setText("退出");

    /* 摄像头 */
    camera = new Camera(this);

    /* 信号连接槽 */
    connect(pushButton[1], SIGNAL(clicked(bool)), camera, SLOT(cameraProcess(bool))); /*转换状态*/
...
void MainWindow::setButtonText(bool bl)
{
    ...
    else
    {
        /* 若关闭了摄像头则禁用拍照按钮 */
        pushButton[0]->setEnabled(false);
        pushButton[1]->setText("开始");  /* 即显示开始时不可以拍照 */
    }
}

void MainWindow::saveImageToLocal()
{
    qDebug()<<"路径："<<QCoreApplication::applicationDirPath()<<endl;
    /* 判断图像是否为空 */
    if (!saveImage.isNull())
    {
